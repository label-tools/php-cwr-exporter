...
if (!function_exists('assertCwrRule18DetailOrderV22')) {
    /**
     * @param string[] $lines
     */
    function assertCwrRule18DetailOrderV22(array $lines): void
    {
        $orderGroups = [
            ['NWR', 'REV', 'ISW', 'EXC'],
            ['SPU'],
...
it('renders detail records in the correct Rule 18 order for v22', function () {
    $cwr = CwrBuilder::v22()
        ->senderType(SenderType::PUBLISHER)
        ->senderId('01234567890')
        ->senderName('Order Validator')
        ->software('LabelTools CWR Exporter', '0.1')
        ->transaction(TransactionType::NEW_WORK_REGISTRATION->value)
        ->works([
            [
                'submitter_work_number' => 'ORDERTEST2',
                'title' => 'Rule 18 Order',
